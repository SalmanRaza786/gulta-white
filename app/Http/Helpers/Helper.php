...
class Helper
{
   ...
   public static function getMimeType($fileName)
    ...
    public static function generateUniqueNumber($digits)
    {
        // Ensure the input is valid
        if (!is_numeric($digits) || $digits <= 0) {
            throw new \InvalidArgumentException("Number of digits must be a positive integer.");
        }

        // Generate a random number with the specified digits
        $min = (int) str_pad('1', $digits, '0'); // Minimum value based on digits
        $max = (int) str_pad('', $digits, '9');  // Maximum value based on digits

        // Generate the random number within the range
        $uniqueNumber = random_int($min, $max);

        // Optionally, ensure the uniqueness of the number
